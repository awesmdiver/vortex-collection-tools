  ...
  fetch('/api/settings')
    ...
    .then((cfg) => {
      if (!cfg.staging || !cfg.downloads) {
        ...
        // was ever configured) -- confirmed real 2026-08-16: the banner above says "fill in the
        // paths below," but without this the rail could land on any other category entirely,
        // making that claim false. settings-app.js loads before this callback ever fires (same
        // deferred-to-DOMContentLoaded technique used elsewhere in this file), so the export is
        // already there to call.
        window.showSettingsCategory?.('paths');
      } else {
        showToolArea('home');
      }
    })
