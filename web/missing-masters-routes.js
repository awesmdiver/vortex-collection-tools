...
function createMissingMastersRouter(config) {
    ...
    // Rebuilds ONE mod (not a whole collection) from Vortex's own state.v2 record + its intact
    // archive -- offered when a "missing" master's own staging folder was found completely empty
    // (see missing-masters-scan.js's hollowInstalls/possibleHollowInstall). Reuses Rebuild
    // Collection's own classify/extract/verify/swap engine unchanged (lib/rebuild-single-mod.js).
