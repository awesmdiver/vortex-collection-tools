...
const DEFAULT_CONFIG = {
    ...
    // Merge engine selection (2026-08-24, merge-port-implement) -- default flipped to TRUE the same
    // day: v2 (lib/merge-v2-worker.js, the zMerge port) is now the real path every merge runs
    // through, not a dev-only comparison flag. Proven byte-identical to zMerge on the director's own
    // real 30-plugin scenario, with the known BuildReferences crash root-caused and fixed. Set to
    // `false` in config.json for an instant, real rollback to the old engine
    // (lib/merge-worker.js/merge-runner.js/merge-preflight.js -- untouched by the port, still fully
    // reachable this way) if v2 needs to be pulled from production without a code change. No
    // Settings UI toggle exists for this on purpose; it's an escape hatch, not a user-facing choice.
    // See docs/plans/2026-08-24-merge-port-spec.md.
    mergeUseV2Engine: true,
