...
// The 'ascii' writes in this file are correct and deliberate, not the mirror of the read-side
// encoding bug fixed in esp-header.js (2026-08-23). `sig`/'TES4' are fixed 4-byte ASCII tokens by the
// format's own definition, and DUMMY_AUTHOR is a hardcoded ASCII constant. Nothing here writes real
// string DATA -- in particular this builder never emits a MAST subrecord, because a dummy master by
// definition declares no masters of its own.
//
// If that ever changes -- if anything here starts writing a filename, author or description that
// isn't guaranteed ASCII -- it must be encoded as Windows-1252, NOT 'ascii' (which silently masks the
// high bit off every byte) and NOT 'utf8' (which would emit two bytes where the format expects one,
// producing a subrecord whose declared size no longer matches its content). See esp-header.js's own
// decodeWin1252 for the read-side counterpart.
