...
// loop already uses).
//
// The mod-identity CROSS-REFERENCE below deliberately does NOT reuse collection-diff.js's own
// buildIndex/findMatch wholesale -- only its modIdentityKeys. That pair's last-resort bare-modId
// fallback key is correct for collection-diff.js's own real purpose ("is this an update of the same
// mod across revisions" -- a version bump keeps modId but changes fileId, so the fallback is what lets
// an update match at all). It's WRONG here: two different fileIds under the same Nexus mod page are
// two genuinely DIFFERENT files (e.g. a mod's several separate optional/patch downloads), and removing
// one must never be blocked because another collection happens to use a DIFFERENT file from that same
// page. Confirmed as a real, live false positive 2026-08-19 against the director's own real
// collections: "GTS CE - Player Homes Patches" and four sibling patches (all distinct fileIds under
// Nexus modId 141199) were flagged as shared with "Body Swap updated"/"My GTS Audio Overhaul" purely
// via the bare-modId fallback; Vortex's own UI does not consider them shared, and the director
// confirmed live. buildSharedModIndex/findSharedModMatch below are the same md5/fileId/logicalName
// priority chain minus that one fallback key -- a real file/logical-name match still counts as shared,
// a same-mod-page-different-file coincidence no longer does.

const fs = require('fs');
const path = require('path');
const { modIdentityKeys } = require('./collection-diff');
