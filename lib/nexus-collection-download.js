...
function resolveApiKey() {
    const candidates = [
        path.join('E:', 'SteamLibrary', 'steamapps', 'common', 'Skyrim Special Edition', 'tools', '.nexus_api_key'),
        process.env.NEXUS_API_KEY_FILE,
    ].filter(Boolean);
    for (const f of candidates) {
        if (fs.existsSync(f)) return fs.readFileSync(f, 'utf8').trim();
    }
    if (process.env.NEXUS_API_KEY) return process.env.NEXUS_API_KEY.trim();
    throw new Error('No Nexus API key found (checked the Skyrim install\'s tools/.nexus_api_key and $NEXUS_API_KEY).');
}
...
module.exports = { resolveApiKey, fetchCollectionRevision, fetchCollectionRevisions, resolveRealDownloadUrl, downloadToFile, fetchAndExtractCollectionJson };
