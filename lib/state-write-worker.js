...
// Neither apply-ignores-write nor apply-disables-write builds a modIndex for its own purposes (Rules
// Generator's own apply-write does, and reuses that copy directly instead of calling this) -- so
// this file's own automatic Cycle Helper snapshot trigger builds one fresh, from the same already-
// open live db, right before writing. See lib/cycle-helper-snapshot.js's captureFromLiveDb for the
// shared logic and full rationale (best-effort, never blocks the real write, must run BEFORE it).
async function captureCycleHelperSnapshot(db, syncLib) {
...
async function main() {
    ...
    switch (mode) {
        ...
        // Follow-up freshness check ONLY -- never used for a real backup's actual displayed/saved
        // numbers (the real backup, via a SEPARATE, earlier 'backup-capture' call in its OWN worker
        // process, has already succeeded and returned by the time sync-runner.js's
        // checkBackupFreshness even starts). Safe to do BOTH reads (safe + WAL-included) in this one
        // process specifically because neither result here is ever the authoritative data the user
        // sees -- if the WAL-included read crashes this whole worker, checkBackupFreshness's own
        // try/catch just treats it as "couldn't check," same as any other failure; nothing
        // authoritative was ever at risk. One process instead of two also halves how many child
        // processes (and any window-flash from each) a single Create Backup click produces.
        case 'backup-capture-freshness-check': {
            const { collectionModId, profileId } = input;
            const captureOnce = async (db) => {
                const rules = await syncLib.getRules(db, collectionModId);
                const ignored = syncLib.extractIgnored(rules);
                let disabled = [];
                if (profileId) {
            ...
            const nameSet = (list) => new Set((list || []).map((m) => m.name));
            ...
            result = {
                disabledDiff: symmetricDiffCount(nameSet(safe.disabled), nameSet(walIncluded.disabled)),
                ignoredDiff: symmetricDiffCount(nameSet(safe.ignored), nameSet(walIncluded.ignored)),
            };
            break;
        }
        ...
        case 'apply-ignores-write': {
            ...
            result = await syncLib.withLiveStateDb(stateDir, async (db) => {
                // Captured BEFORE the write below -- the whole point is "what did rules look like
                // right before this change," not after.
                await captureCycleHelperSnapshot(db, syncLib);
                return await syncLib.writeIgnoredFlags(db, modId, ignoredRefs);
            });
            break;
        }
        ...
        case 'apply-disables-write': {
            ...
            result = await syncLib.withLiveStateDb(stateDir, async (db) => {
                // Captured BEFORE the write below -- same reasoning as apply-ignores-write above.
                // writeDisabledFlags only touches modState###enabled, not the rules array itself,
                // but capturing before-vs-after either write in this file is the simpler, uniform
                // rule to follow rather than reasoning per-write about which fields it touches.
                await captureCycleHelperSnapshot(db, syncLib);
                const { results: matches, identityWarning } = await syncLib.findCurrentModIdsChecked(db, disabledRefs);
