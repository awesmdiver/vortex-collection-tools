...
async function main() {
    ...
    switch (mode) {
        case 'list-profiles': {
            result = await syncLib.withStateDb(stateDir, async (db) => ({ profiles: await syncLib.listProfiles(db) }));
            break;
        }
        ...
        case 'apply-ignores-preview': {
            ...
            result = await syncLib.withStateDb(stateDir, async (db) => {
                ...
                return { changed: syncLib.applyIgnoresToRules(rules, ignoredRefs).changed };
            });
            break;
        }
        ...
        case 'apply-ignores-write': {
            ...
            result = await syncLib.withLiveStateDb(stateDir, async (db) => ({
                changed: await syncLib.writeIgnoredFlags(db, modId, ignoredRefs),
            }));
            break;
        }
        ...
        case 'apply-disables-preview': {
            ...
            result = await syncLib.withStateDb(stateDir, async (db) => {
                const matches = await syncLib.findCurrentModIds(db, disabledRefs);
                return { matches };
            });
            break;
        }
        ...
        case 'apply-disables-write': {
            ...
            result = await syncLib.withLiveStateDb(stateDir, async (db) => {
                const matches = await syncLib.findCurrentModIds(db, disabledRefs);
                return { changed: await syncLib.writeDisabledFlags(db, profileId, matches) };
            });
            break;
        }
        default:
