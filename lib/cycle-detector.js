...
function makeLevelDbRuleIO(db) {
  return {
    async readRules(modId) {
      ...
      // undefined (no rules key at all) hits assertRulesShapeKnown's own "not an array" guard below
      // for a clear message -- classic-level 3.0.0 yields undefined rather than throwing
      // LEVEL_NOT_FOUND for a missing key, see lib/vortex-sync/lib.js's own comment on the same fix.
