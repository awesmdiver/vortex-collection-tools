...
async function retryModExtraction({ collectionModId, staging, downloads, state, modId }) {
    ...
    if (u) {
        ...
        if (rebuildResult.status !== 'REBUILT') {
            return { ok: false, name: u.new.name, error: describeApplyFailure(rebuildResult), status: rebuildResult.status || rebuildResult.kind, code: rebuildResult.code };
        }
    ...
    if (rebuildResult.status !== 'REBUILT') {
        return { ok: false, name: m.name, error: describeApplyFailure(rebuildResult), status: rebuildResult.status || rebuildResult.kind, code: rebuildResult.code };
    }
        ...
        try {
            ...
            if (rebuildResult.status !== 'REBUILT') {
                ...
                finishUpdated({
                    ...
                    status: rebuildResult.status || rebuildResult.kind, code: rebuildResult.code,
                });
                continue;
            }
