...
async function applyFixViaHelper(ownerModKey, ruleType, targetModKey, action) {
    console.log(`[cycle-helper-runner] applyFixViaHelper: owner="${ownerModKey}" ruleType=${ruleType} target="${targetModKey}" action=${action}`);
    const data = await helperClient.getAllMods();
    if (!data) {
        console.warn('[cycle-helper-runner] applyFixViaHelper: getAllMods() returned nothing -- helper unreachable, falling back to state.v2 path.');
        return null;
    }
    const modIndex = rg.buildModIndexFromLiveData(data.mods, data.enabledModKeys);
    const fixResult = await cd.applyCandidateFix(makeHelperRuleIO(), modIndex, ownerModKey, ruleType, targetModKey, action);
    console.log(`[cycle-helper-runner] applyFixViaHelper: real write committed -- originalRule=${JSON.stringify(fixResult.originalRule)}`);
    ...
    //
    // FIXED (2026-09-04, real confirmed bug -- director live-caught it: Validate reported "still
    // cycling", showing BOTH the old and new rule direction as live candidates, right after a fix
    // Vortex's own real state had already correctly applied). Root cause, confirmed via the logging
    // added for this same investigation: `fixResult.originalRule` comes from `applyCandidateFix`'s
    // own INDEPENDENT `ruleIO.readRules()` fetch (a separate HTTP round-trip from the `getAllMods()`
    // call this function already did above to build `modIndex`) -- so it can never be the SAME
    // object instance as anything in `ownerEntry.rules`, even when their content is identical. A
    // reference-equality lookup (`indexOf`) was therefore guaranteed to fail on EVERY call, not just
    // when a duplicate rule happened to be present -- it just went unnoticed until a failure actually
    // produced a visibly wrong result. Fixed by matching on VALUE instead (same `type` + same
    // resolved target), mirroring EXACTLY the lookup `applyCandidateFix` itself already uses to find
    // the real rule to remove (cycle-detector.js's own `applyCandidateFix`, the `currentRules.findIndex`
    // call) -- this function already has `ruleType`/`targetModKey` in scope, the same two values that
    // real lookup keys on, so no new resolution logic is needed here.
    const ownerEntry = modIndex.get(ownerModKey);
    const rulesBeforeCount = ownerEntry.rules.length;
    // Duplicate-rule check, kept (see cycle-detector.js's own rankCulpritEdges/dedupe comment for the
    // real, confirmed root cause class this guards against: a mod can carry SEVERAL separate rule
    // objects that all resolve to the same current target because Vortex never replaces an old
    // version-pinned rule on update, it just piles a new one on -- fixing/removing only ONE of them
    // leaves the edge intact and a real cycle genuinely remains). Now matches by the SAME value-based
    // key the real removal below uses, so "duplicate" means "a second rule the fix didn't touch",
    // not an artifact of the old reference-equality bug.
    const matchesTarget = (r) => r.type === ruleType && rg.resolveRefToModKey(modIndex, r.reference) === targetModKey;
    const removeIdx = ownerEntry.rules.findIndex(matchesTarget);
    const duplicateRules = ownerEntry.rules.filter((r, i) => i !== removeIdx && matchesTarget(r));
    if (duplicateRules.length > 0) {
        console.warn(`[cycle-helper-runner] applyFixViaHelper: owner "${ownerModKey}" has ${duplicateRules.length} OTHER rule(s) of the same type also resolving to "${targetModKey}" -- this fix only touches one rule object, so the edge this cycle depends on may still exist via a duplicate. Duplicates: ${JSON.stringify(duplicateRules)}`);
    }
    if (removeIdx === -1) {
        // Shouldn't happen -- the real commit above already found and removed a matching rule via the
        // identical value-based lookup, so this modIndex (read moments before that write) should
        // always have at least one match too. Logged as a real anomaly, not treated as routine.
        console.warn(`[cycle-helper-runner] applyFixViaHelper: value-based lookup ALSO found no match for type=${ruleType} target="${targetModKey}" inside ownerEntry.rules (length ${rulesBeforeCount}) -- unexpected, the real commit found one moments ago. Local validation will run without removing anything.`);
    }
    ...
    console.log(`[cycle-helper-runner] applyFixViaHelper: local validation model -- rules before=${rulesBeforeCount} after=${updatedRules.length} (removeIdx=${removeIdx}, added=${!!newRule})`);

    const analysis = cd.analyzeCycles(modIndex, data.enabledModKeys, null);
    const validation = shapeCycleResult(rg, modIndex, analysis);
    console.log(`[cycle-helper-runner] applyFixViaHelper: predicted validation -- hasCycles=${validation.hasCycles}${validation.hasCycles ? ` cycles=${JSON.stringify(validation.cycles)}` : ''}`);
    return {
        ...fixResult,
        targetModKey,
