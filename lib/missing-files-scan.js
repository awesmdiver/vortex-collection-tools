...
async function scanOneMod(mod, { downloadsDir, stagingDir, sevenZipExe, exceptionMatcher = () => false, knownVortexModId = null }) {
    const base = { name: mod.name, modId: mod.source?.modId ?? null, fileId: mod.source?.fileId ?? null };
    let action;
    try {
        action = await classifyMod(mod, { downloadsDir, stagingDir, knownVortexModId, sevenZipExe, exceptionMatcher });
    } catch (e) {
