...
// Resolves the exact set of archive-relative destination paths that SHOULD exist in this mod's
// staging folder, replaying the same logic extract-mod.js uses for a real rebuild. Returns
// { destinations: string[] } on success, or { unresolvable: reason } when this mod can't be
// reliably resolved without actually extracting (an unhandled FOMOD feature) -- never guesses.
async function resolveExpectedDestinations(mod, archiveEntries, archivePath, sevenZipExe) {
    if (mod.choices?.type === 'fomod') {
        ...
        return { destinations: files.map((f) => f.destination) };
    }
...
//   { bucket: 'missing', name, modId, fileId, targetFolderName, archivePath, missing: string[] }
...
async function scanOneMod(mod, { downloadsDir, stagingDir, sevenZipExe }) {
    const base = { name: mod.name, modId: mod.source?.modId ?? null, fileId: mod.source?.fileId ?? null };
    let action;
    try {
        action = await classifyMod(mod, { downloadsDir, stagingDir, knownVortexModId: null, sevenZipExe });
    } catch (e) {
    ...
    const missing = resolved.destinations.filter((dest) => !fs.existsSync(path.join(action.stagingModDir, dest)));
...
async function scanCollections(collectionModIds, { stagingDir, downloadsDir, sevenZipExe, onProgress }) {
    const results = [];
    for (const collectionModId of collectionModIds) {
        ...
        for (let i = 0; i < mods.length; i++) {
            ...
            let result;
            try {
                result = await scanOneMod(mod, { downloadsDir, stagingDir, sevenZipExe });
            } catch (e) {
        ...
        results.push({ collectionModId, name, modsWithMissing, modsArchiveMissing });
    }
    return results;
}
