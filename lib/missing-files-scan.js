...
async function scanOneMod(mod, { downloadsDir, stagingDir, sevenZipExe }) {
    ...
    const missing = resolved.destinations.filter((f) => !fs.existsSync(path.join(action.stagingModDir, f.destination)));
...
async function scanCollections(collectionModIds, { stagingDir, downloadsDir, sevenZipExe, onProgress, ignoredMatchers }) {
    const results = [];
    for (const collectionModId of collectionModIds) {
        ...
        results.push({ collectionModId, name, modsWithMissing, modsArchiveMissing, modsIgnored });
    }
    return results;
}
