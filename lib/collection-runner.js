...
async function runRebuild({ rebuildQueue, collectionJsonPath, downloadsDir, stagingDir, modEntries, onModStart, onModComplete, concurrency = 1 }) {
    let haltedCritical = false;
    let nextIndex = 0;

    async function worker() {
        while (!haltedCritical) {
...
// Vortex open.
async function resolveMismatchedMod({ collectionModId, stagingDir, downloadsDir, modId, fileId, targetFolderName, resolveMode }) {
    ...
    if (!fs.existsSync(stagingModDir)) {
        throw new Error(`Expected an existing staging folder at "${stagingModDir}" (from the original run's own log) but it's gone.`);
    }
