...
// handles one collection per invocation.
async function loadSyncState({ state, collectionModId, collection }) {
    const results = await runIsolatedStateQuery({
        syncLibPath: SYNC_LIB_PATH, state, collections: [{ modId: collectionModId, collection }],
    });
...
async function buildPlan({ removedMods, keptMods, knownVortexModIds, resumed, downloadsDir, stagingDir, sevenZipExe, otherVersionsByModId, onModClassified }) {
    const modEntries = [];
    const rebuildQueue = [];
    ...
    for (const mod of keptMods) {
        ...
        if (action.kind === 'SKIP_NO_ARCHIVE') {
            ...
            const entry = { ...base, status: 'SKIP_OPEN_FOMOD', detail: otherVersionsNote ? `A different version of this exact mod IS installed: ${otherVersionsNote}` : undefined };
            modEntries.push(entry);
            if (onModClassified) onModClassified(entry, index, total);
        } else {
            ...
            rebuildQueue.push({ mod, action, base: { ...base, otherVersionsNote: otherVersionsRebuildNote || undefined } });
            if (onModClassified) {
