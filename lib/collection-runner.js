...
const { locateArchive, resolveDomainDownloadsDir } = require('./archive-locator');
...
const RESUMABLE_STATUSES = new Set([
    'REBUILT', 'SKIP_NO_ARCHIVE', 'SKIP_OPTIONAL_NOT_INSTALLED', 'SKIP_OPEN_FOMOD', 'SKIP_EXCEPTED',
...
// ---- Helper-backed equivalent of state-query-worker.js's own logic (2026-08-21) ----
// Reads Vortex's live in-memory state via the Vortex Collection Helper extension instead of opening
// state.v2 directly, so Vortex never needs to be closed for this (see
// design/mockup-rebuild-collection-helper-support.html -- approved, no UI change, purely backend).
// Reuses every PURE function syncLib already exports (extractIgnored, computeSync,
// makeIdentityMatcher) completely unchanged; only the parts of state-query-worker.js that actually
// iterate LevelDB keys (findCurrentModIds' full-scan match, buildModVersionIndex, scanAllCollections)
// are reimplemented below against the Helper's own getAllMods() payload -- confirmed by reading the
// Helper's real server-side handler (vortex-collection-helper/index.js's getAllMods) that it returns
// the WHOLE `state.persistent.mods[gameId]` subtree unfiltered ("dumb relay, no logic"), so every
// field state-query-worker.js reads via individual DB key gets (attributes.fileMD5/referenceTag/
// modId/fileId/customFileName/collectionSlug/name/source/revisionNumber, installationPath, type,
// rules) is already present on each mod object in ONE HTTP call -- no new Helper endpoint needed.
// Produces the EXACT SAME raw per-collection shape state-query-worker.js's own `out[modId]` does
// (plus the same `__workshopOnly` key), so loadSyncState/loadSyncStateBatch's own toSyncStateResult
// mapping works completely unchanged regardless of which path produced the raw data -- callers never
// need to know which path ran.
//
// KNOWN, DELIBERATE DIFFERENCE from the state.v2 path, flagged rather than silently worked around:
// state-query-worker.js's buildModVersionIndex scans EVERY Vortex profile's own modState (enumerating
// `persistent###profiles###*###modState###*###enabled` across ALL profiles) to build each mod's
// enabledProfiles list. The Helper's getAllMods() only exposes enabledModKeys for the CURRENTLY
// ACTIVE profile (state.persistent.profiles[activeProfileId].modState only) -- there is no existing
// Helper endpoint for "enabled state across every profile". A mod that's enabled only in a different,
// INACTIVE profile will therefore show enabledProfiles: [] here (rendered as "installed but not
// enabled" by describeOtherVersions) where the state.v2 path would correctly say "enabled". This only
// affects the DISPLAY TEXT of the "a different version is already installed" note -- it never affects
// any actual classify/rebuild decision (those never read enabledProfiles at all) -- and multi-profile
// support is already a known, separately-tracked gap elsewhere in this project (see TODO.md's own
// "Multi-profile validation" item), so this isn't introducing a new class of problem, just inheriting
// an existing one on one additional display string.
function refKeyLive(ref) {
    return `${ref.modId}:${ref.fileId}`;
}

function ruleIdentityKeysLive(rule) {
...
// Mirrors syncLib.findCurrentModIds's full-scan match (findCurrentModIdsCore's fallback branch --
// the ONLY branch ever actually exercised by this caller, since it never has a pre-existing
// vortexModId to fast-path on -- same reasoning as state-query-worker.js's own header comment).
// Reads identity straight off the already-fetched mods object instead of iterating LevelDB keys.
function findCurrentModIdsLive(allMods, refs, syncLib) {
    const matcher = syncLib.makeIdentityMatcher(refs);
    const results = [];
    for (const [key, mod] of Object.entries(allMods)) {
        const attrs = mod.attributes || {};
        if (attrs.fileMD5 == null) continue;
        const identity = { fileMD5: attrs.fileMD5, tag: attrs.referenceTag, modId: attrs.modId, fileId: attrs.fileId };
        const match = matcher(identity);
        if (match) results.push({ vortexModId: key, matchedRef: match });
    }
    return results;
}

// Mirrors state-query-worker.js's buildModVersionIndex -- see this section's own header comment for
// the one known, deliberate difference (enabledProfiles is active-profile-only here).
function buildModVersionIndexLive(allMods, enabledModKeys) {
    ...
    for (const [key, mod] of Object.entries(allMods)) {
        ...
        const info = {
            key, modId: attrs.modId, version: attrs.version, fileId: attrs.fileId, variant: attrs.variant,
            installationPath: mod.installationPath,
            enabledProfiles: enabledSet.has(key) ? [key] : [],
        };
        if (!byModId.has(attrs.modId)) byModId.set(attrs.modId, []);
        byModId.get(attrs.modId).push(info);
    }
    return byModId;
}

// Mirrors state-query-worker.js's scanAllCollections exactly (same membership algorithm -- fileMD5
// first, then repo.modId+repo.fileId, see that function's own header comment for why this matches
// Vortex's real generateCollectionMap) -- reads from the already-fetched mods object instead of a
// full LevelDB iteration.
function scanAllCollectionsLive(allMods) {
    const workshopOnly = [];
    const membership = new Map();
    for (const [key, mod] of Object.entries(allMods)) {
        if (mod.type !== 'collection') continue;
        const attrs = mod.attributes || {};
        const collectionName = attrs.customFileName || attrs.name || key;
        const rules = Array.isArray(mod.rules) ? mod.rules : [];
        for (const rule of rules) {
            for (const idKey of ruleIdentityKeysLive(rule)) {
        ...
        workshopOnly.push({
            name: collectionName, source: attrs.source || null, modId: key, folder: mod.installationPath || key,
            collectionSlug: attrs.collectionSlug || null, localRevisionNumber: attrs.revisionNumber ?? null,
        });
    }
    return { workshopOnly, membership };
}

// Line-for-line port of state-query-worker.js's own main() body (the withStateDb callback), reading
// from `allMods` (one getAllMods() call) instead of a live db handle. Produces the identical `out`
// shape (per-modId {ignored, removedMods, keptMods, knownVortexModIdEntries, otherVersionsByModId,
// sharedWithCollectionsByKey, liveName, collectionSlug} plus __workshopOnly) that
// runIsolatedStateQuery's own JSON result does, so toSyncStateResult works on either unchanged.
async function queryLiveStateBatch({ helperClient, syncLib, collections, stagingDir }) {
    const data = await helperClient.getAllMods();
    if (!data) throw new Error('Could not read live mod state from the Vortex Collection Helper.');
    ...
    for (const { modId, collection } of collections) {
        try {
            ...
            if (!mod) {
                throw new Error(
                    `No mod with id "${modId}" found under game "${syncLib.GAME_ID}" in Vortex's live state. ` +
                    'This usually means Vortex hasn\'t written this mod to its state yet. Try either: ' +
            ...
            const attrs = mod.attributes || {};
            perCollection[modId] = {
                ok: true, ignored, removedMods, keptMods,
                liveName: attrs.customFileName, collectionSlug: attrs.collectionSlug,
            };
            for (const km of keptMods) {
                // Same "browse"-source skip as state-query-worker.js's own comment -- no stable
                // Vortex-tracked identity to look up for a non-Nexus mod anyway.
                if (km.source.modId == null || km.source.fileId == null) continue;
                allRefs.push({ modId: km.source.modId, fileId: km.source.fileId, fileMD5: km.source.md5, tag: km.source.tag });
            }
        } catch (e) {
    ...
    const matches = allRefs.length > 0 ? findCurrentModIdsLive(allMods, allRefs, syncLib) : [];
    const matchByKey = new Map(matches.map((m) => [refKeyLive(m.matchedRef), m.vortexModId]));
    const modVersionIndex = buildModVersionIndexLive(allMods, data.enabledModKeys);
    const scan = stagingDir ? scanAllCollectionsLive(allMods) : { workshopOnly: [], membership: new Map() };

    const out = {};
    for (const { modId } of collections) {
        const pc = perCollection[modId];
        if (!pc.ok) { out[modId] = { error: pc.error }; continue; }
        const knownVortexModIdEntries = pc.keptMods
            .map((mod) => {
                const key = refKeyLive({ modId: mod.source.modId, fileId: mod.source.fileId });
...
// Helper-backed equivalent of loadSyncState -- same signature/return shape, caller doesn't need to
// know which path ran. `helperClient`/`syncLib` are passed in (not required at the top of this file)
// to keep this module exactly as framework-agnostic as it already was -- the CALLER (web/rebuild-
// routes.js, which already loads both) decides whether Vortex is even reachable this way at all.
async function loadSyncStateViaHelper({ helperClient, syncLib, collectionModId, collection, stagingDir }) {
    const results = await queryLiveStateBatch({
        helperClient, syncLib, collections: [{ modId: collectionModId, collection }], stagingDir,
    });
    return toSyncStateResult(results[collectionModId]);
}

// Helper-backed equivalent of loadSyncStateBatch -- same signature/return shape.
async function loadSyncStateBatchViaHelper({ helperClient, syncLib, entries, stagingDir }) {
    const raw = await queryLiveStateBatch({ helperClient, syncLib, collections: entries, stagingDir });
    const results = new Map();
    for (const { modId } of entries) {
        try {
            results.set(modId, { ok: true, data: toSyncStateResult(raw[modId]) });
...
// Same reasoning as getOpenFomodMods above -- a mod on the Mod Exceptions list (queue: rebuild-
// missing-hand-pick-exceptions) is a deliberate, standing opt-out, worth its own callout on the
// Plan page rather than blending into the general summary.
function getExceptedMods(modEntries) {
    return modEntries.filter((e) => e.status === 'SKIP_EXCEPTED');
}
...
async function buildPlan({ collectionModId, removedMods, keptMods, knownVortexModIds, resumed, downloadsDir, stagingDir, sevenZipExe, otherVersionsByModId, sharedWithCollectionsByKey, logsDir, onModClassified, downloadMissingArchivesEnabled = false, forceExtractOffSiteMismatchesEnabled = false, exceptionMatcher = () => false }) {
    const modEntries = [];
    const rebuildQueue = [];
    ...
    for (const mod of keptMods) {
        ...
        const action = await classifyMod(mod, {
            downloadsDir, stagingDir, knownVortexModId, collectionModId, sevenZipExe,
            forceExtractOffSiteMismatch: forceExtractOffSiteMismatchesEnabled,
            overrideArchivePath, exceptionMatcher,
        });
        ...
        if (action.kind === 'SKIP_NO_ARCHIVE') {
            ...
            } else if (mod.source?.type === 'bundle') {
                // Bundle-type mods (2026-08-26 fix) are intercepted inside classifyMod() before
                // locateArchive() ever runs -- action.code here is one of BUNDLE_PACKAGE_NOT_FOUND/
                // BUNDLE_FOLDER_NOT_FOUND, never HASH_MISMATCH/NOT_FOUND, and action.detail already
                // carries the real, honest reason (lib/bundle-resolver.js's own error text). Checked
                // BEFORE the generic `mod.source?.type !== 'nexus'` branches below -- 'bundle' would
                // otherwise also match those (it's not 'nexus' either) and get the WRONG, misleading
                // "download manually from: <no URL recorded>" message a bundle mod can never satisfy
                // (bundle sources carry a fileExpression, never a url).
            ...
            modEntries.push(entry);
            if (onModClassified) onModClassified(entry, index, total);
        } else if (action.kind === 'SKIP_EXCEPTED') {
            // Mod Exceptions list (queue: rebuild-missing-hand-pick-exceptions) -- never touched by
            // this run, archive not even hashed (see classifyMod's own early-return for why).
            const entry = { ...base, status: 'SKIP_EXCEPTED', detail: action.detail };
            modEntries.push(entry);
            if (onModClassified) onModClassified(entry, index, total);
        } else {
...
async function retryMissingArchiveDownload({ collectionModId, stagingDir, downloadsDir, sevenZipExe, modId, fileId }) {
    ...
    try {
        // mod.domainName takes priority over the collection-wide default -- same per-mod override
        // fix as web/rebuild-missing-routes.js's Download Archive (commit a12787a) and
        // downloadMissingArchivesForPlan's own batch loop (lib/nexus-mod-download.js) -- a mod
        // cross-listed under a different game's own Nexus catalog carries its own domainName in
        // collection.json, distinct from the collection's overall info.domainName.
        const gameDomain = mod.domainName || collectionInfo.collection.info?.domainName;
        // Same per-mod resolution on the FILESYSTEM side as gameDomain is for the API side -- a
        // cross-listed mod's archive belongs in ITS OWN domain's downloads folder. See
        // resolveDomainDownloadsDir's own header comment.
        const destDir = resolveDomainDownloadsDir(downloadsDir, mod.domainName);
        await nexusModDownload.downloadModArchive({
            apiKey, gameDomain, source: mod.source, destDir,
        });
    } catch (e) {
...
async function reclassifyMod({ collectionModId, stagingDir, downloadsDir, sevenZipExe, modId, fileId, name }) {
    ...
    const action = await classifyMod(mod, { downloadsDir, stagingDir, knownVortexModId: null, collectionModId, sevenZipExe });
    if (action.kind !== 'REBUILD') {
        return {
            kind: action.kind, detail: action.detail, code: action.code,
            candidateFile: action.candidateFile, candidateFiles: action.candidateFiles,
...
async function forceExtractOffSiteMod({ collectionModId, stagingDir, downloadsDir, sevenZipExe, name }) {
    ...
    const action = await classifyMod(mod, { downloadsDir, stagingDir, knownVortexModId: null, collectionModId, sevenZipExe, forceExtractOffSiteMismatch: true });
    if (action.kind !== 'REBUILD') {
...
async function extractImportedOffSiteMod({ collectionModId, stagingDir, downloadsDir, sevenZipExe, name }) {
    ...
    const action = await classifyMod(mod, { downloadsDir, stagingDir, knownVortexModId: null, collectionModId, sevenZipExe, overrideArchivePath });
    if (action.kind !== 'REBUILD') {
        // code/candidateFile/candidateFiles must travel too, not just detail -- same stale-code bug
...
module.exports = {
    ...
    loadSyncState,
    loadSyncStateBatch,
    loadSyncStateViaHelper,
    loadSyncStateBatchViaHelper,
    loadResumeLog,
    writeLog,
    summarize,
    getOpenFomodMods,
