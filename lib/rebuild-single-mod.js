...
const { buildModFromVortexState } = require('./build-mod-from-vortex-state');
...
async function rebuildSingleMod({ vortexModId, gameId, stateDir, downloadsDir, stagingDir, allowAutoDownload = false }) {
    ...
    const mod = await buildModFromVortexState({ stateDir, gameId, vortexModId });
