...
async function rebuildMod(mod, action, { collectionJsonPath, downloadsDir, stagingDir }, resolveMode) {
    ...
    if (resolveMode === 'keep-existing') {
        ...
        // but iterating ours's ORIGINAL-cased keys so the actual copy below reads a real path under
        // rebuildingDir, not a lowercased one that may not exist on disk.
        const theirsLower = new Set(Object.keys(theirs).map((k) => k.toLowerCase()));
        const missingFromStaging = Object.keys(ours).filter((p) => !theirsLower.has(p.toLowerCase()));
    ...
    return {
        ...
        note: resolveMode === 'all' && hasMismatch
            ? `Manually resolved (full replace): archive version installed outright, discarding `
