...
function scanStagingCollections(stagingDir) {
  ...
  for (const entry of entries) {
    ...
    // "__vortex_tmp_<n>" (2026-08-30, real bug caught live: "Update Collection Test Collection"
    // showing TWICE on Update Collection v2's own picker, one as `collection_8` -- the real,
    // correctly-named collection -- and one as `__vortex_tmp_00000001`, same name, same revision,
    // same mod count). This is Vortex's own internal staging name for a folder it extracts a fresh
    // install/update INTO before renaming it to the real `<Name>-<modId>-<version>-<timestamp>`
    // folder -- never a Workshop collection, never a second real install, just a transient artifact.
    // The one seen live was a stale leftover from an interrupted/repeated apply-revert testing
    // cycle, still sitting on disk with a real, fully-populated collection.json inside it, so the
    // plain filesystem scan below picked it up as if it were legitimate. Unlike the Workshop-folder
    // case just below, this pattern is NEVER a real second collection regardless of what's inside
    // it -- no hasRealWorkshopContent-style content check needed, unconditional skip.
    if (/^__vortex_tmp_/i.test(entry.name)) continue;
...
async function getVortexAppVersion(db) {
  // classic-level 2.0+ (abstract-level 2.0's "not found" change) yields undefined for a missing key
  // instead of throwing LEVEL_NOT_FOUND -- confirmed via UPGRADING.md, 2026-08-27's classic-level
  // 1.4.1 -> 3.0.0 upgrade. Guard the JSON.parse directly rather than relying on a thrown error.
...
async function writeIgnoredFlags(db, modId, ignoredRefs) {
  ...
  // undefined (no rules key at all for this mod) hits assertRulesShapeKnown's own "not an array"
  // guard below for a clear message -- see getVortexAppVersion's own comment on why this can't be
  // JSON.parse(raw) unconditionally anymore (classic-level 3.0.0, 2026-08-27).
  const rules = raw === undefined ? undefined : JSON.parse(raw);
...
async function writeDisabledFlags(db, profileId, matches) {
  ...
  for (const { vortexModId, matchedRef } of matches) {
    ...
    // db.get() itself already yields undefined for a missing key (classic-level 3.0.0) -- no catch
    // needed, see getVortexAppVersion's own comment.
...
async function getLastActiveProfileId(db, gameId = GAME_ID) {
  // See getVortexAppVersion's own comment -- db.get() yields undefined directly now, doesn't throw.
