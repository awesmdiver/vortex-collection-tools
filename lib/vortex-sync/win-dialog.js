...
// Async save-file picker -- same non-blocking spawn() reasoning as pickFolderAsync/pickOpenFileAsync
// above (Save Cleaner's own "Save As" button, called from an Express route). Ports pickSaveFile's
// synchronous SaveFileDialog body (showDialog, save:true) onto the async spawn shape, since
// pickSaveFile itself is only ever called from the interactive CLI menu today and blocking an
// Express request on a user's dialog interaction would freeze the whole app the same way the async
// folder/open-file pickers were already written to avoid.
function pickSaveFileAsync({ title = 'Save file', initialDir, filter = 'All files (*.*)|*.*', defaultFileName } = {}) {
  const lines = [
    'Add-Type -AssemblyName System.Windows.Forms | Out-Null',
    ...OWNER_FORM_LINES,
    ...
    `$dlg.Title = ${psSingleQuote(title)}`,
    `$dlg.Filter = ${psSingleQuote(filter)}`,
    // Deliberately left at .NET's own default (true) -- this is the actual mechanism behind the
    // mockup's own "nothing is ever overwritten without you choosing it in the dialog" promise: the
    // OS itself asks "Do you want to replace it?" the moment an existing filename is picked, before
    // this code ever sees the result. Our own backup-before-write and ReSaver's own ESS.writeESS
    // backup are both separate, additional safety nets on top of this, not a replacement for it.
...
module.exports = { pickOpenFile, pickSaveFile, pickFolderAsync, pickOpenFileAsync, pickSaveFileAsync };
