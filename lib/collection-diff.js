...
function modIdentityKeys(mod) {
    ...
    // md5, for a source with none of the above at all (2026-08-30, real live bug: a `type: 'direct'`
    // mod -- GitHub-hosted, not Nexus -- has no modId/fileId and no logicalFilename, so it produced
    // ZERO identity keys and could never match itself across revisions. Every single review showed it
    // as a genuine Remove+Add, forever, regardless of whether the file had actually changed at all --
    // confirmed live, "MaxsuBlockOverhaul-v0.22.7z", the exact same md5 landing THREE separate times
    // in local collection.json because its own "removed" copy never successfully left before the next
    // review appended another "added" one. Deliberately scoped to ONLY fire when every other signal is
    // absent (never overrides modId+fileId/logicalFilename/nexus, all checked first above) -- md5
    // alone is a real content-identity signal, just a weaker one than this file's own header comment
    // already explains for why it can't be promoted above those for mods that DO have them (a genuine
    // update commonly changes md5, so trusting it as a PRIMARY key would misclassify an update as
    // unrelated Remove+Add instead -- but for a source with nothing else to go on, "same content" is
    // strictly better than "never matches at all").
    if (keys.length === 0 && src.md5) keys.push(`md5:${src.md5}`);
    return keys;
}
