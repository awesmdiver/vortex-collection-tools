...
// Two pure, data-source-agnostic lookup builders (2026-08-18, same "one algorithm, two data
// sources" pattern as Cycle Helper's buildModIndex/buildModIndexFromLiveData and Rules Generator's
// ruleIO) -- each takes a plain Map already normalized to the flat shape state.v2's own keys
// describe (modId/downloadId -> {field: value}), regardless of whether that Map was built from a
// LevelDB iteration (readModsAndDownloads, below) or from the helper extension's already-fetched
// live /mods + /downloads responses (modInfoFromLiveMods/downloadInfoFromLiveFiles, below). Kept
// separate rather than one combined function since scanStaging only ever needs the mod half,
// scanArchives/crossCheck need varying combinations of both -- see each call site.
function buildModLookup(modInfo) {
    ...
    return { modsByInstallPath, usedArchiveIds };
}

function buildDownloadLookup(downloadInfo) {
    ...
    for (const [downloadId, info] of downloadInfo) {
        ...
        downloadLocalPaths.add(info.localPath);
    ...
    return { downloadLocalPaths, downloadsByLocalPath };
}

// Live-helper equivalents of the DB iteration below -- normalize the helper extension's real nested
// Redux shapes (confirmed live 2026-08-18 against a real running Vortex: archiveId/installationPath
// are top-level fields on a mod record, collectionSlug lives under attributes -- exactly mirroring
// state.v2's own flattened `mods###modId###archiveId` / `###installationPath` /
// `###attributes###collectionSlug` key split) into the SAME flat Map shape the DB path builds, so
// buildModLookup/buildDownloadLookup work identically regardless of data source.
function modInfoFromLiveMods(mods) {
    const modInfo = new Map();
    for (const modKey of Object.keys(mods || {})) {
        const m = mods[modKey] || {};
        modInfo.set(modKey, {
            installationPath: m.installationPath,
            archiveId: m.archiveId,
            collectionSlug: m.attributes ? m.attributes.collectionSlug : undefined,
        });
    }
    return modInfo;
}

function downloadInfoFromLiveFiles(files) {
    const downloadInfo = new Map();
    for (const downloadId of Object.keys(files || {})) {
        const f = files[downloadId] || {};
        downloadInfo.set(downloadId, { localPath: f.localPath, state: f.state, game: f.game });
    }
    return downloadInfo;
}
...
async function readModsAndDownloads(db) {
    ...
    return { ...buildModLookup(modInfo), ...buildDownloadLookup(downloadInfo) };
}
...
// Pure classification, data-source-agnostic (2026-08-18) -- takes the `{modsByInstallPath}` lookup
// buildModLookup produces (from EITHER a state.v2 DB iteration or the helper extension's live /mods
// response) and classifies staging folders identically regardless of which one built it.
function classifyStagingFolders({ modsByInstallPath }, stagingDir, ignoredNames) {
...
async function scanStaging(stateDir, stagingDir, ignoredNames = []) {
    ...
    return syncLib.withStateDb(stateDir, async (db) => {
        const lookup = await readModsAndDownloads(db);
        return classifyStagingFolders(lookup, stagingDir, ignoredNames);
    });
}

// Live-helper equivalent -- `mods` is the raw `data.mods` object from a caller's own already-made
// vortexHelperClient.getAllMods() call, no DB/Vortex-closed requirement. Synchronous (no I/O once
// the caller has the data in hand), unlike scanStaging's own async/DB-open shape.
function scanStagingFromLiveData(mods, stagingDir, ignoredNames = []) {
    const { modsByInstallPath } = buildModLookup(modInfoFromLiveMods(mods));
    return classifyStagingFolders({ modsByInstallPath }, stagingDir, ignoredNames);
}

// Same pattern as classifyStagingFolders -- pure, data-source-agnostic, takes the
// `{usedArchiveIds, downloadLocalPaths, downloadsByLocalPath}` lookup buildModLookup +
// buildDownloadLookup produce together.
function classifyArchiveFiles({ usedArchiveIds, downloadLocalPaths, downloadsByLocalPath }, downloadsDir, ignoredNames) {
    ...
    // on usedArchiveIds in buildModLookup. Real example that grounded this: "OWL -
...
async function scanArchives(stateDir, downloadsDir, ignoredNames = []) {
    ...
    return syncLib.withStateDb(stateDir, async (db) => {
        const lookup = await readModsAndDownloads(db);
        return classifyArchiveFiles(lookup, downloadsDir, ignoredNames);
    });
}

// Live-helper equivalent -- needs BOTH `mods` (data.mods, for usedArchiveIds) and `files`
// (data.files from the new vortexHelperClient.getAllDownloads(), for the download-side lookups),
// from the caller's own already-made calls. Synchronous, same reasoning as scanStagingFromLiveData.
function scanArchivesFromLiveData(mods, files, downloadsDir, ignoredNames = []) {
    const { usedArchiveIds } = buildModLookup(modInfoFromLiveMods(mods));
    const { downloadLocalPaths, downloadsByLocalPath } = buildDownloadLookup(downloadInfoFromLiveFiles(files));
    return classifyArchiveFiles({ usedArchiveIds, downloadLocalPaths, downloadsByLocalPath }, downloadsDir, ignoredNames);
}

// Pure classification shared by crossCheck/crossCheckFromLiveData -- `lookup` is EITHER
// `{modsByInstallPath}` (when checking the staging/dir side) or `{downloadLocalPaths}` (when
// checking the archives/file side), whichever `kind`/`otherKind` below actually needs -- see each
// caller for which one it builds and passes.
//
...
// only a real Vortex-state check can confirm that.
function classifyCrossCheck(lookup, kind, deletedNames, otherDir) {
    ...
    for (const name of candidates) {
        const stillOrphaned = otherKind === 'dir'
            ? (() => { const mod = lookup.modsByInstallPath.get(name); return !mod || (!mod.archiveId && !mod.collectionSlug); })()
            : !lookup.downloadLocalPaths.has(name);
        if (stillOrphaned) matches.push({ name });
    }
    return matches;
}

// Requires Vortex closed, same as scanStaging/scanArchives (this is why crossCheck is async and
// needs stateDir).
async function crossCheck(stateDir, kind, deletedNames, otherDir) {
    const syncLib = loadSyncLib();
    return syncLib.withStateDb(stateDir, async (db) => {
        const lookup = await readModsAndDownloads(db);
        return classifyCrossCheck(lookup, kind, deletedNames, otherDir);
    });
}

// Live-helper equivalent -- only ever needs ONE side's live data (mods for kind==='archives',
// downloads for kind==='staging' -- see classifyCrossCheck's own otherKind logic), so this only
// fetches whichever one it actually needs rather than always paying for both (mirrors the
// extension's own /downloads being a separate endpoint specifically so a mods-only caller doesn't
// pay for it -- see that endpoint's own comment). `mods`/`files` -- pass whichever one applies,
// `null` for the other; the unused one is never read.
function crossCheckFromLiveData(mods, files, kind, deletedNames, otherDir) {
    const lookup = kind === 'archives'
        ? buildModLookup(modInfoFromLiveMods(mods || {}))
        : buildDownloadLookup(downloadInfoFromLiveFiles(files || {}));
    return classifyCrossCheck(lookup, kind, deletedNames, otherDir);
}
...
module.exports = {
    ...
    scanArchives,
    crossCheck,
    scanStagingFromLiveData,
    scanArchivesFromLiveData,
    crossCheckFromLiveData,
    deleteEntries,
};
