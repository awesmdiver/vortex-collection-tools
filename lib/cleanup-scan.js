...
// auto-generated download-naming shape. Deliberately conservative: a real download whose version
// string contains letters (e.g. "1.0RC2") won't match either, but that just routes it to
// needsReview instead of exceptions -- the safe direction, never the reverse.
const RECOGNIZED_DOWNLOAD_NAME_PATTERN = /-\d+(?:-\d+)*-\d{10}$/;
// A SECOND, different Vortex-generated naming shape, confirmed real 2026-07-27 -- a file the user
// manually downloaded from Nexus's website (or otherwise moved straight into the downloads folder,
// bypassing Vortex's own download manager) instead of using Vortex's "Download with Manager"
// button. Space-separated, ends in an ISO-ish timestamp ("2026-07-27T19-43Z") plus a random
// alphanumeric suffix (e.g. "Portalmaster 186656 1.0 2026-07-27T19-43Z hfH9s6oFY.rar") -- modId/
// version ordering varies between examples, so only the trailing date+suffix shape is matched.
// This exact shape can ALSO appear on a real Vortex-tracked download (confirmed earlier this same
// session: "Variadic Collision Dynamics..." had a genuine downloads### record) -- it is NOT proof
// of being an orphan by itself, only a naming-convention hint. Only matters for needsReview items
// (no downloads### record at all): those get labeled "Possible manual download?" instead of the
// generic "unrecognized format" message, since the likely explanation differs from a hand-named
// tool-output folder (DynDOLOD Output, etc).
...
function isRecognizedDownloadName(name) {
    return RECOGNIZED_DOWNLOAD_NAME_PATTERN.test(name);
}
