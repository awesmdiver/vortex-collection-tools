...
function makeLevelDbRuleIO(db) {
  return {
    async readRules(modId) {
      // classic-level 3.0.0 yields undefined for a missing key instead of throwing LEVEL_NOT_FOUND
      // (2026-08-27 upgrade, see lib/vortex-sync/lib.js's own comment) -- a mod with no rules key at
      // all is the common case (most mods have never had a rule written), so this still needs to
      // resolve to [] rather than let assertRulesShapeKnown reject it as malformed.
      const raw = await db.get(`persistent###mods###${GAME_ID}###${modId}###rules`);
      const rules = raw === undefined ? [] : JSON.parse(raw);
...
async function applyRules(ruleIO, modIndex, oldCollectionKey, newCollectionKey, ruleOverrides, anomalyOverrides, relationshipOverrides, { dryRun = false, anomalyMemory = { choices: {} }, stagingRoot = null } = {}) {
  ...
  for (const [newModKey, rulesToAdd] of byMod) {
    ...
    // -- ruleIO.readRules returns `[]` for that case (see makeLevelDbRuleIO's own undefined-key
    // handling above), same convention vortex-sync/lib.js's own field readers already follow.
