...
function scanMissingMasters(dataDir, pluginsTxtPath, stagingDir, dummyMastersOutputDir, eslifierOutputDir) {
    ...
    // Plugins we genuinely COULDN'T CHECK (2026-08-23). Skipping an unreadable plugin is right --
    // we can't read it and guessing would be worse -- but skipping it SILENTLY was the bug: the scan
    // reported "no missing or disabled masters found" while one of the user's plugins had never been
    // looked at. If that was the plugin with the broken dependency, we told them everything was fine
    // while their game still crashed.
    //
    // Deliberately its own return field rather than folded into problemMasters: "we couldn't check
    // this" is a different kind of thing from "this is broken", and every action the problem list
    // offers (create a dummy, rebuild the mod, open staging) is meaningless for a plugin we never
    // parsed. Reason codes only -- the wording lives in the frontend, same split as everywhere else.
    const unreadable = []; // [{ name, reason }]
    let scanned = 0;
    for (const [key, info] of filesOnDisk) {
        // NOT reported below -- an inactive plugin is deliberately out of scope (Skyrim never loads
        // it, so it can't crash anything), not something we failed to read. It's also excluded from
        // `scanned`, which is why this sits above the counter.
        ...
        try {
            ...
            unreadable.push({ name: info.actualFileName, reason: 'read-error', detail: e.message });
            continue;
        }
        // readPluginHeader returns null when the file isn't a valid TES4 record at all (shorter than
        // a 24-byte header, or the signature isn't "TES4") -- a truncated or genuinely corrupt file.
        if (!header) {
            unreadable.push({ name: info.actualFileName, reason: 'invalid-header' });
            continue;
        }
        // Compressed TES4 (flag 0x00040000) -- esp-header.js detects but deliberately doesn't
        // inflate. `masters` is null in exactly this case, which is why the two are checked together.
        if (header.compressed || !header.masters) {
            unreadable.push({ name: info.actualFileName, reason: 'compressed-header' });
            continue;
        }
        // NOT reported -- a plugin declaring zero masters is healthy and extremely common (Skyrim.esm
        // itself, and any standalone plugin). There is simply nothing here to check, which is not the
        // same as having failed to check it.
        if (header.masters.length === 0) continue;
    ...
    // Stable order so the silent background poll's own JSON-snapshot comparison doesn't see a
    // spurious change from fs.readdirSync's platform-dependent ordering.
    unreadable.sort((a, b) => a.name.localeCompare(b.name));

    return { total: scanned, problemMasters, unreadable };
}
...
module.exports = {
    ...
    // Exported 2026-08-21 for web/pgpatcher-routes.js's own DynDoLOD-owning-mod resolution (the real
    // disable -> build -> re-enable flow) -- the exact same "which real Vortex mod owns this plugin
    // file right now" lookup this file's own scanMissingMasters already relies on, reused rather than
    // reimplemented.
