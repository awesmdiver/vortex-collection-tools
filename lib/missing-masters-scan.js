...
// Vortex Scrub uses for its own exceptions/needsReview classification. Confirmed real 2026-07-27
...
// against Vortex Scrub's own safety use first: a hand-named tool-output folder never ends in a
...
function scanMissingMasters(dataDir, pluginsTxtPath, stagingDir, dummyMastersOutputDir) {
    ...
    function findActiveAlternate(masterFileName) {
        ...
        for (const siblingKey of siblings) {
            ...
            if (active.has(siblingKey)) {
                ...
                return { name: onDisk.actualFileName, modName: trueSourceModName, sameModAsMaster: trueSourceModName === modName };
            }
        }
        return null;
    }
