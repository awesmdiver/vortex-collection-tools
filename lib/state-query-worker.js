...
async function scanAllCollections(db, stagingDir) {
    ...
    for await (const [key, value] of db.iterator()) {
        ...
        if (m) {
            // "__vortex_tmp_<n>" (2026-08-30) -- same real gap scanStagingCollections' own header
            // comment documents in lib/vortex-sync/lib.js, hit here too: this function discovers
            // collections purely by live `type === 'collection'`, not a filesystem scan, but a stale
            // `__vortex_tmp_*` staging folder from an interrupted apply gets a full, real-looking
            // live mod record too -- confirmed live, `state: "installed"`, `type: "collection"`,
            // even the SAME real `collectionSlug` as the genuine collection it's a leftover artifact
            // of. Never a real second collection regardless of what its live record says, so this
            // gets the same unconditional skip -- no Workshop-style content check, that distinction
            // doesn't apply to this pattern at all.
            if (/^__vortex_tmp_/i.test(m[1])) continue;
